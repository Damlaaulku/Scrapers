...
class Redfin:
    ...
    # Parcel Requests
    def flood_zone(self, property_id, listing_id, **kwargs):
        parcel_info = self.property_parcel(property_id, listing_id)
        apn = parcel_info['payload']['apn']
        locId = parcel_info['payload']['locId']
        try:
            fipsCode = parcel_info['payload']['fipsCode']
        except:
            fipsCode = ""
        return self.meta_request('api/home/details/flood/floodInfoFipsApnLocId', {'fipsCode': fipsCode, 'apn': apn, 'locId': locId, **kwargs})
